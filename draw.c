...
/*======== void is_visible() ==========
  ...
  double z2  
  Returns: 
  Calculates the angle between view vector and surface vector of given triangle to determine if the surface is visible
  ====================*/
int is_visible(double x0, double y0, double z0,
	       double x1, double y1, double z1,
	       double x2, double y2, double z2){
	
  double v1[] = {x1 - x0, y1 - y0, z1 - z0};
  double v2[] = {x2 - x0, y2 - y0, z2 - z0};
  double view[] = {0, 0, -1};
  ...
  result = normal[0] * view[0] + normal[1] * view[1] + normal[2] * view[2];
  return result < 0;
}
...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  ...
  for (i = 0; i < polygons -> lastcol - 2; i += 3){
    if (is_visible(polygons->m[0][i], polygons->m[1][i], polygons->m[2][i],
...
void add_sphere( struct matrix * points, 
  ...
  int latStop, longStop, latStart, longStart, max;
  latStart = 0;
  latStop = num_steps;
  longStart = 0;
  longStop = num_steps;
  max = num_steps * (num_steps + 1);
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
	...
	add_polygon(points, 
		    ...
		    temp->m[1][ index ],
		    temp->m[2][ index ],
		    temp->m[0][ (index + 1) % max ],
		    temp->m[1][ (index + 1) % max ],
		    temp->m[2][ (index + 1) % max ],
		    temp->m[0][ (index + num_steps + 1) % max ],
		    temp->m[1][ (index + num_steps + 1) % max ],
		    temp->m[2][ (index + num_steps + 1) % max ]);
      }
	...
	add_polygon(points, 
		    ...
		    temp->m[1][ (index + 1) % max ],
		    temp->m[2][ (index + 1) % max ],
		    temp->m[0][ (index + num_steps) % max ],
		    temp->m[1][ (index + num_steps) % max ],
		    temp->m[2][ (index + num_steps) % max ]);
      }
...
void add_torus( struct matrix * points, 
  ...
  int latStop, longtStop, latStart, longStart, max;;
  latStart = 0;
  longStart = 0;
  latStop = num_steps;
  longtStop = num_steps;
  max = num_steps * num_steps;
  for ( lat = latStart; lat < latStop; lat++ ) {
	...
	add_polygon(points, 
		    ...
		    temp->m[1][ index ], 
		    temp->m[2][ index ], 
		    temp->m[0][ (index + 1 - num_steps) % max ],
		    temp->m[1][ (index + 1 - num_steps) % max ],
		    temp->m[2][ (index + 1 - num_steps) % max ],
		    temp->m[0][ (index + 1) % max ],
		    temp->m[1][ (index + 1) % max ],
		    temp->m[2][ (index + 1) % max ]);
...
void generate_torus( struct matrix * points, 
  ...
  for ( rotation = rotStart; rotation < rotStop; rotation += step ) {
    ...
    for ( circle = circStart; circle < circStop; circle += step ) {
      ...
      z = sin( 2 * M_PI * rot ) * ( r1 * cos( 2 * M_PI * circ ) + r2 );
